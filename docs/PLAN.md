...
- **⚠️ Cobertura incompleta por diseño — no es un defecto de la importación:** el DCAH prioriza capitales y localidades de 50,000+ habitantes; no cubre el 100% de las colonias de SEPOMEX. **Corrida real de `import/2_dcah_geo.php` contra una BD de prueba completa: 33,118 de 79,775 polígonos emparejados (41.5%)** — un cruce previo por nombre en Python había estimado ~53%, pero el número real medido corriendo el import de verdad (que usa un índice `clave_municipio + nombre` exacto, con la última colonia de cada clave repetida ganando) es este. Es más bajo que el ideal >75% que se documentaba antes — normal al cruzar dos catálogos independientes por nombre; candidato a mejorar después con matching más flexible (por CP, por similitud de texto) si hace falta más cobertura. Mientras tanto sigue existiendo el fallback de M3.4 (centroide aproximado) y el campo `metodo` de `/geolocate`.
...
| M4.3 | Revisar `dcah_sin_match.txt` — cobertura real medida corriendo el import completo: **41.5%** (33,118 de 79,775). Es esperado (cobertura DCAH incompleta + diferencias de nomenclatura entre catálogos), no es un bug | 🟢 | Porcentaje de cobertura documentado |
...
| ✅ M4.V | `SELECT COUNT(*) FROM colonia_poligonos` — con el ~41% de match esperado, revisar que la cifra ronde las ~33,000 filas, no que sea 0 o el 100% de las colonias | ✅ | |
